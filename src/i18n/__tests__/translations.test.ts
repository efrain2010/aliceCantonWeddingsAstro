...
describe('translations structural parity', () => {
  ...
  it('footer has the same keys in both locales', () => {
    expect(Object.keys(en.footer).sort()).toEqual(Object.keys(es.footer).sort());
  });
  ...
  it('all page sections have the same keys in both locales', () => {
    const pageKeys = ['home', 'about', 'services', 'venues', 'gallery', 'contact'] as const;
    for (const key of pageKeys) {
      expect(Object.keys(en.pages[key]).sort(), `pages.${key} keys differ`).toEqual(
        Object.keys(es.pages[key]).sort()
      );
    }
  });
  ...
  it('about values items, process steps arrays are the same length', () => {
    expect(en.pages.about.values.items.length).toBe(es.pages.about.values.items.length);
    expect(en.pages.about.process.steps.length).toBe(es.pages.about.process.steps.length);
    expect(en.pages.about.venues.buttons.length).toBe(es.pages.about.venues.buttons.length);
  });
  ...
  it('services items and values items arrays are the same length', () => {
    ...
    expect(en.pages.services.values.items.length).toBe(es.pages.services.values.items.length);
  });

  it('services details arrays are the same length per item', () => {
    en.pages.services.items.forEach((item, i) => {
      expect(item.details.length, `services.items[${i}].details length differs`).toBe(
        es.pages.services.items[i].details.length
      );
    });
  });
  ...
  it('contact faq items array is the same length', () => {
    expect(en.pages.contact.faq.items.length).toBe(es.pages.contact.faq.items.length);
  });
});
...
describe('translations completeness', () => {
  ...
  it('no empty strings in Spanish translations', () => {
    function check(obj: unknown, path = '', key = '') {
      ...
      else if (Array.isArray(obj)) obj.forEach((v, i) => check(v, `${path}[${i}]`));
      else if (typeof obj === 'object' && obj !== null)
        Object.entries(obj).forEach(([k, v]) => check(v, path ? `${path}.${k}` : k, k));
    }
    check(es);
  });
